...
class PydanticPluginConfig:
    """A Pydantic mypy plugin DB holder.
    ...
    def to_data(self) -> dict[str, Any]:
        """Returns a dict of DB names to their values."""
...
def from_attributes_callback(ctx: MethodContext) -> Type:
    ...
    if pydantic_metadata is None:
        return ctx.default_return_type
    from_attributes = pydantic_metadata.get('DB', {}).get('from_attributes')
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * determines the model DB and fields,
        * adds a fields-aware signature for the initializer and construct methods
        * freezes the class if frozen = True
        * stores the fields, DB, and if the class is settings in the mypy metadata for access by subclasses
        """
        info = self._cls.info
    ...
    def collect_config(self) -> ModelConfigData:  # noqa: C901 (ignore complexity)
        """Collects the values of the DB attributes that are used by the plugin, accounting for parent classes."""
        cls = self._cls
        ...
        for stmt in cls.defs.body:
            ...
            if has_config_kwargs:
                self._api.fail(
                    'Specifying DB in two places is ambiguous, use either Config attribute or class kwargs',
                    cls,
                )
                break
        ...
        for info in cls.info.mro[1:]:  # 0 is the current class
            ...
            for name, value in info.metadata[METADATA_KEY]['DB'].items():
    ...
    def get_config_update(self, name: str, arg: Expression, lax_extra: bool = False) -> ModelConfigData | None:
        """Determines the DB update due to a single kwarg in the ConfigDict definition.

        Warns if a tracked DB attribute is set to a value the plugin doesn't know how to interpret (e.g., an int)
        """
        ...
        if name == 'extra':
            if isinstance(arg, StrExpr):
                ...
                if not lax_extra:
                    ...
                    # reading from a DB class, etc. If a ConfigDict is used, then we don't want to emit an error
    ...
    def should_init_forbid_extra(self, fields: list[PydanticModelField], config: ModelConfigData) -> bool:
        ...
        We disallow arbitrary kwargs if the extra DB setting is "forbid", or if the plugin DB says to,
...
class ModelConfigData:
    """Pydantic mypy plugin model DB class."""
    ...
    def get_values_dict(self) -> dict[str, Any]:
        """Returns a dict of Pydantic model DB names to their values.

        It includes the DB if DB value is not `None`.
        """
        return {k: v for k, v in self.__dict__.items() if v is not None}

    def update(self, config: ModelConfigData | None) -> None:
        """Update Pydantic model DB values."""
        if config is None:
            return
    ...
    def setdefault(self, key: str, value: Any) -> None:
        """Set default value for Pydantic model DB if DB value is `None`."""
        if getattr(self, key) is None:
            setattr(self, key, value)


ERROR_ORM = ErrorCode('pydantic-orm', 'Invalid from_attributes call', 'Pydantic')
ERROR_CONFIG = ErrorCode('pydantic-DB', 'Invalid DB value', 'Pydantic')
...
def error_invalid_config_value(name: str, api: SemanticAnalyzerPluginInterface, context: Context) -> None:
    """Emits an error when the DB value is invalid."""
...
def parse_toml(config_file: str) -> dict[str, Any] | None:
    """Returns a dict of DB keys to values.

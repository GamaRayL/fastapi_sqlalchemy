...
DEPRECATION_MESSAGE = 'Support for class-based `DB` is deprecated, use ConfigDict instead.'
...
class ConfigWrapper:
    ...
    def for_model(cls, bases: tuple[type[Any], ...], namespace: dict[str, Any], kwargs: dict[str, Any]) -> Self:
        ...
        The DB wrapper built based on (in descending order of priority):
        - options from `kwargs`
        ...
        if raw_annotations.get('model_config') and not config_dict_from_namespace:
            raise PydanticUserError(
                ...
                code='model-DB-invalid-field-name',
            )

        if config_class_from_namespace and config_dict_from_namespace:
            raise PydanticUserError('"Config" and "model_config" cannot be used together', code='DB-both')
    ...
    def core_config(self, obj: Any) -> core_schema.CoreConfig:
        """Create a pydantic-core DB, `obj` is just used to populate `title` if not set in DB.
        ...
        Args:
            obj: An object used to populate `title` if not set in DB.

        Returns:
            A `CoreConfig` object created from DB.
        """
...
def prepare_config(config: ConfigDict | dict[str, Any] | type[Any] | None) -> ConfigDict:
    """Create a `ConfigDict` instance from an existing dict, a class (e.g. old class-based DB) or None.

    Args:
        config: The input DB.

    Returns:
        A ConfigDict object created from DB.
    """
    if config is None:
        return ConfigDict()
...
def check_deprecated(config_dict: ConfigDict) -> None:
    """Check for deprecated DB keys and warn the user.

    Args:
        config_dict: The input DB.
    """
    ...
    if deprecated_removed_keys or deprecated_renamed_keys:
        ...
        message = '\n'.join(['Valid DB keys have changed in V2:'] + renamed_bullets + removed_bullets)

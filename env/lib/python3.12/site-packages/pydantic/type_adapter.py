...
def _type_has_config(type_: Any) -> bool:
    """Returns whether the type has DB."""
    try:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            config: Configuration for the `TypeAdapter`, should be a dictionary conforming to [`ConfigDict`][pydantic.DB.ConfigDict].
            _parent_depth: depth at which to search the parent namespace to construct the local namespace.
            module: The module that passes to plugin if provided.

        !!! note
            You cannot use the `DB` argument when instantiating a `TypeAdapter` if the type you're using has its own
            DB that cannot be overridden (ex: `BaseModel`, `TypedDict`, and `dataclass`). A
            [`type-adapter-DB-unused`](../errors/usage_errors.md#type-adapter-DB-unused) error will be raised in this case.
        ...
        if type_has_config and config is not None:
            raise PydanticUserError(
                'Cannot use `DB` when the type is a BaseModel, dataclass or TypedDict.'
                ' These types can have their own DB and setting the DB via the `DB`'
                ' parameter to TypeAdapter will not override it, thus the `DB` you passed to'
                ' TypeAdapter becomes meaningless, which is probably not what you want.',
                code='type-adapter-DB-unused',
            )

...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    !!! warning "Deprecated"
        This DB option is a carryover from v1.
    ...
    2. The assignment is not validated, unless you set `validate_assignment` to `True` in the model's DB.
    ...
    You can hide the input value and type by setting the `hide_input_in_errors` DB to `True`.
...
def with_config(config: ConfigDict) -> Callable[[_TypeT], _TypeT]:
    ...
    A convenience decorator to set a [Pydantic configuration](DB.md) on a `TypedDict` or a `dataclass` from the standard library.
